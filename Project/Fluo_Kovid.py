...
class Tracker():
    def __init__(self, algo, maxDisappeared=0): #set maxDisappeared=0 means the object will be deregistered immediately once not detected.
        ...
        self.centroid_paths = OrderedDict() # added to trace trajectory of each object
        self.objects_contours = OrderedDict() # keep a record or the contour of each object
        self.objects_under_mitosis = OrderedDict() # recording if the cell is under mitosis
        self.objects_area_when_mitosis = OrderedDict() # record a list of contour area for cells undergoes mitosis		
...
def diplay_frame_with_wait(frame, waiting):
    global f1, global_cell_count, global_mitosis_count, sf
    # plt.figure(figsize=(7,7))
    # plt.imshow(frame)
    # plt.show()
    _,name = f1.split('\\')
    # print(name)
    path = 'C:/Users/Kovid/Documents/UNSW/2020 Term 2/COMP9517/Project/Output/' + sf +'/'
    # print(os.path.join(path, name))
    txt = f'Mitosis Count = {global_mitosis_count}'
    xval, yval = 930, 20
    frame[yval-19:yval+33, xval-5:xval+169] = 0
...
def draw_bbox_on_input(image, all_bbox, status):
    image = copy.deepcopy(image)
    #define 2 colors, only the first color is used for now
    RGBforLabel = { False:(0,255,255), True:(255,69,20) }
...
def mitosis_detection(obj_id, contour):
    min_required_compactness = 1.25
    max_area = 500
    min_intensity_requirement = frame_mean_intensity+80
    under_mitosis = False
    ...
    mean_val = cv2.mean(normalized,mask = mask)[0]
    # print('\nobj_id=',obj_id)
    ...
    # print(obj_id)
    # print(contour_area)
    if compactness > min_required_compactness and mean_val > min_intensity_requirement and contour_area < max_area:
...
def mitosis_finished(obj_id, contour, old_area):
    #assume mitosis finishes in the next frame unless its shape its area is larger than a threshold
    area_thresh = 450
    contour_area = cv2.contourArea(contour)
    # print('~~~~~~~~~')
    # print(obj_id)
...
"========================Main=================================================="
n = '1'
Fluo_folder = f'Fluo-N2DL-HeLa/Sequence {n}/*.tif'
sf = f'0{n}'
...
for k in range(0,len(img_files)):
    ...
    out_img = cv2.cvtColor(adathr1,cv2.COLOR_GRAY2BGR)
    frame_mean_intensity = cv2.mean(main)[0]
    
    #get contours from segmented image
    contours = get_contour_canny(main)
    ...
    print('Number of mitosed cells so far =',mitosis_count)
    print('------------------')
