...
class Tracker():
    ...
    '''
    def __init__(self, algo, maxDisappeared=5):
    ...
    def centroid_update(self, rects):
        '''
		...
		# centroids
            ...
            #Kovid uncomment below 3 lines
            # else:
            #     for col in unusedCols:
            #         self.register(inputCentroids[col])
		# return the set of trackable objects
        return self.objects, self.centroid_paths
'''
...
def diplay_frame_with_wait(frame, waiting):
    ...
    print(name)
    path = 'C:/Users/Kovid/Documents/UNSW/2020 Term 2/COMP9517/Project/Output'
...
def contour_processing(all_contours):
    out_contours = []
    min_area_requirement = 50 #hard-coded number for now, feel free to change - size
...
'''
def draw_bbox_on_input(image, all_bbox):
    image = copy.deepcopy(image)
    #define 2 colors, only the first color is used for now
    RGBforLabel = { 1:(0,0,255), 2:(0,255,255) }
    ...
    print('Number of contours=',len(all_bbox))
    return image
...
Fluo_folder = 'Fluo-N2DL-HeLa-Kovid/Sequence 1/*.tif'
Fluo_mask_folder = 'Fluo-N2DL-HeLa/Sequence 1 Masks/*.tif'
# DIC_folder = 'DIC-C2DH-HeLa/Sequence 1/*.tif'
# DIC_mask_folder = 'DIC-C2DH-HeLa/Sequence 1 Masks/*.tif'
# DIC_folder = 'DIC-C2DH-HeLa/01/*.tif'
# DIC_mask_folder = 'DIC-C2DH-HeLa/01_RES/*.tif'
...
mask_files = glob.glob(Fluo_mask_folder)
mask_files.sort()
...
# for k in range(3):
    filename = img_files[k]
    # mask1 = mask_files[k]
    print(f'\nFilename = {filename}', end='')
    #open input and segmented image
    main = cv2.imread(filename,cv2.IMREAD_GRAYSCALE)
    # main = cv2.normalize(main, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    main[main > 1] = 255
    main[main < 1] = 0
    # main = cv2.dilate(main, None)
    # main = cv2.erode(main, None)
    # main = cv2.bitwise_not(main)
    kernel = (3,3)
    main = cv2.erode(main,kernel,iterations = 15)
    main = cv2.dilate(main,kernel,iterations = 15)
    main = cv2.bitwise_not(main)
    ...
    contours = get_contours(main)
    #post-processing contour to remove unneeded ones
    ...
    objects, trajectorys = centroid_tracker.update(formatted_rects)
    ...
    bbox_image = draw_bbox_on_input(out_img, bboxes)
